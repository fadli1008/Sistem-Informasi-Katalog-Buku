...
def init_db():
    ...
    # Migrasi Kolom jika user sudah punya database lama agar tidak error
    try:
        c.execute("ALTER TABLE peminjaman ADD COLUMN tanggal_jatuh_tempo TEXT")
...
if menu == "🔍 Katalog Buku":
    ...
    query = f'''
        SELECT b.id_buku as ID, b.cover_url as 'Sampul', b.judul as 'Judul Buku', b.pengarang as 'Pengarang', 
               ...
               END as 'Status'
        FROM buku b
        LEFT JOIN peminjaman p ON b.id_buku = p.id_buku AND p.status_pinjam IN ('Dipinjam', 'Menunggu Pengembalian')
        WHERE 1=1
    '''
    params = []
...
# ------------------------------------------
    ...
    if st.session_state['role'] != 'Siswa':
        ...
        with t2:
            ...
            q_riwayat = f'''
                SELECT b.judul as 'Judul Buku', p.tanggal_pinjam as 'Tanggal Pinjam', p.tanggal_jatuh_tempo as 'Batas Kembali',
                       CASE 
                            WHEN p.status_pinjam = 'Dipinjam' AND p.tanggal_jatuh_tempo < '{tgl_hari_ini}' THEN 'Terlambat'
                            ELSE p.status_pinjam 
                       END as 'Status Transaksi',
                       p.denda as 'Denda Berhasil Dibayar (Rp)'
...
# ------------------------------------------
    ...
    if st.session_state['role'] != 'Admin':
        ...
        # TAB 0: Analitik
        with tab0:
            st.markdown("#### 📈 Dasbor Analitik & Statistik Perpustakaan")
            st.caption("Analisis data sirkulasi buku secara real-time untuk laporan manajemen sekolah.")
            st.write("")
            
            c_left, c_right = st.columns(2)
            with c_left:
                with st.container(border=True):
                    st.markdown("##### 🏆 5 Buku Terpopuler")
                    q_populer = 'SELECT b.judul as "Judul Buku", COUNT(p.id_pinjam) as "Total Dipinjam" FROM peminjaman p JOIN buku b ON p.id_buku = b.id_buku GROUP BY b.id_buku ORDER BY [Total Dipinjam] DESC LIMIT 5'
                    df_populer = pd.read_sql_query(q_populer, conn)
                    if df_populer.empty: st.info("Belum ada data sirkulasi peminjaman.")
                    else: st.bar_chart(df_populer.set_index('Judul Buku'), y_label="Kali Dipinjam")
                        
            with c_right:
                with st.container(border=True):
                    st.markdown("##### 🕒 Tren Grafik Total Peminjaman Terdaftar")
                    q_waktu = "SELECT tanggal_pinjam FROM peminjaman"
                    df_waktu = pd.read_sql_query(q_waktu, conn)
                    if df_waktu.empty: st.info("Belum ada data tanggal peminjaman.")
                    else:
                        df_waktu['Jumlah'] = 1
        ...
        with tab1:
            ...
            # SUB-KOLOM 1: Proses Approval Pinjam Baru
            with col_p:
                ...
                if not df_app_p.empty:
                    st.dataframe(df_app_p[['id_pinjam', 'judul', 'nama_siswa']], use_container_width=True, hide_index=True)
                    ...
                    if bp1.button("✅ Setujui", use_container_width=True):
                        ...
                        conn.execute("UPDATE peminjaman SET status_pinjam='Dipinjam', tanggal_pinjam=?, tanggal_jatuh_tempo=? WHERE id_pinjam=?", (tgl_sekarang, tgl_jt, id_p))
                        conn.execute("UPDATE buku SET status='Dipinjam' WHERE id_buku=?", (id_b,))
                        conn.commit()
                        st.toast(f"Disetujui! Jatuh tempo kembali: {tgl_jt}", icon="✅")
                        time.sleep(1)
                        st.rerun()
            ...
            with col_k:
                ...
                if not df_app_k.empty:
                    st.dataframe(df_app_k[['id_pinjam', 'judul', 'nama_siswa']], use_container_width=True, hide_index=True)
                    ...
                    row_sel = df_app_k[df_app_k['id_pinjam'] == id_k].iloc[0]
                    ...
                    if tgl_jatuh_tempo_str:
                        ...
                        if today_date > jt_date:
                            hari_keterlambatan = (today_date - jt_date).days
                            hitung_denda = hari_keterlambatan * 1000
                            st.error(f"⚠️Buku ini TERLAMBAT {hari_keterlambatan} hari! Hitung denda: Rp {hitung_denda:,}")
                        else:
                            st.success("✅ Buku dikembalikan tepat waktu (Bebas Denda).")
                    
                    if st.button("✅ Konfirmasi Buku Diterima", use_container_width=True):
                        conn.execute("UPDATE peminjaman SET status_pinjam='Selesai', denda=? WHERE id_pinjam=?", (hitung_denda, id_k))
                        conn.execute("UPDATE buku SET status='Tersedia' WHERE id_buku=?", (id_bk,))
                        conn.commit()
                        st.toast(f"Sukses! Buku kembali ke rak. Denda dicatat: Rp {hitung_denda:,}", icon="📚")
                        time.sleep(1)
                        st.rerun()
                else:
        ...
        with tab4:
            ...
            q_log = f'''
                SELECT p.id_pinjam as 'ID Transaksi', b.judul as 'Judul Buku', s.nama as 'Nama Peminjam', 
                       s.username as 'NISN', p.tanggal_pinjam as 'Tanggal Pinjam', p.tanggal_jatuh_tempo as 'Batas Tempo',
                       CASE 
                            WHEN p.status_pinjam = 'Dipinjam' AND p.tanggal_jatuh_tempo < '{tgl_hari_ini}' THEN 'Terlambat'
                            ELSE p.status_pinjam 
                       END as 'Status',
            ...
            if df_log.empty:
                ...
                def color_status_log(val):
                    ...
                    elif val == 'Terlambat': color = '#ef4444'
                    else: color = '#3b82f6'
