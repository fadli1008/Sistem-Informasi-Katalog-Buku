...
# ------------------------------------------
    ...
    if st.session_state['role'] != 'Admin':
        ...
        # PERUBAHAN: Menempatkan Tab Analitik di Urutan Pertama Admin Dashboard
        ...
        # TAB 0: IMPLEMENTASI VISUALISASI DATA & DASBOR ANALITIK
        with tab0:
            ...
            with c_left:
                with st.container(border=True):
                    st.markdown("##### 🏆 5 Buku Terpopuler (Paling Sering Dipinjam)")
                    q_populer = '''
                        SELECT b.judul as 'Judul Buku', COUNT(p.id_pinjam) as 'Total Dipinjam'
                        FROM peminjaman p
                        JOIN buku b ON p.id_buku = b.id_buku
                        GROUP BY b.id_buku
                        ORDER BY [Total Dipinjam] DESC
                        LIMIT 5
                    '''
                    df_populer = pd.read_sql_query(q_populer, conn)
                    
                    if df_populer.empty:
                        st.info("Belum ada data sirkulasi peminjaman untuk dianalisis.")
                    else:
                        # Set index ke judul buku agar axis-X grafik membaca teks judul otomatis
                        df_chart_populer = df_populer.set_index('Judul Buku')
                        st.bar_chart(df_chart_populer, y_label="Kali Dipinjam")
                        
            with c_right:
                with st.container(border=True):
                    st.markdown("##### 🕒 Tren Jam Sibuk Kunjungan / Sirkulasi")
                    q_waktu = "SELECT tanggal_pinjam FROM peminjaman"
                    df_waktu = pd.read_sql_query(q_waktu, conn)
                    
                    if df_waktu.empty:
                        st.info("Belum ada data transaksi sirkulasi untuk memetakan jam sibuk.")
                    else:
                        # Parsing string jam dari string format tanggal %Y-%m-%d %H:%M
                        try:
                            df_waktu['Jam'] = pd.to_datetime(df_waktu['tanggal_pinjam'], errors='coerce').dt.hour
                            # Kelompokkan data berdasarkan jam operasi sekolah (07:00 s.d 16:00 atau dinamis)
                            df_hourly = df_waktu.groupby('Jam').size().reset_index(name='Jumlah Aktivitas')
                            
                            # Melengkapi seluruh range jam agar visualisasi runut
                            df_hourly = df_hourly.set_index('Jam').reindex(range(7, 17), fill_value=0)
                            df_hourly.index = df_hourly.index.astype(str) + ":00"
                            
                            st.line_chart(df_hourly, y_label="Total Transaksi")
                        except Exception as e:
                            st.info("Sedang memproses pemetaan grafik waktu...")
